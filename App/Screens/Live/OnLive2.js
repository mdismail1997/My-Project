...
export default function OnLive2(props) {
  const isBroadcaster = props.type === 'create';

  const {profileContextData} = useContext(ProfileContext);

  const onShare = async () => {
    try {
      const result = await Share.share({message: props.channel});
  ...
  useEffect(() => {
    ...
    init().then(() =>
      AgoraEngine.current.joinChannel(agoraToken, props.channel, null, uid),
    );
  ...
  const sendQuestion = async () => {
    console.log('minAmount==>', typeof props.minAmount, props.minAmount);
    console.log('====>', parseInt(messages[index].amount) + props.minAmount);
    if (celebrityAmount < props.minAmount + parseInt(messages[index].amount)) {
      var totalMinAmount = props.minAmount + parseInt(messages[index].amount);
      // setQuestion('');
      //  setCelebrityAmount('');
          ...
          {minAmountStatus && (
            <View
              ...
              <Text
                ...
                The minimum amount for asking question is ${props.minAmount}
              </Text>
