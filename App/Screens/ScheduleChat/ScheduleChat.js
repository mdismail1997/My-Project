...
const ScheduleChat = props => {
  ...
  const sendRequest = async () => {
    ...
    await postApiCall('chat-request-send', requestData, {})
      .then(response => {
  ...
  const videoRequest = async () => {
    if (
      props.route.params.data.chat_request_received ||
      props.route.params.data.chat_request_send ||
      requestSend
    ) {
      if (!item.call_request_received && !item.call_request_send) {
        console.log('+++++++++++All false');
        props.navigation.navigate('ScheduleVideo', {data: item});
      } else {
        setShowWaitModal(true);
      }
    } else {
      Alert.alert('Please send request for chat first');
    }
  };
        ...
        <View
          ...
          <View
            ...
            {/* <View
              ...
              <TextInput
                value={amount}
                onChangeText={value => requestHandel(value)}
                keyboardType="numeric"
                placeholder="Enter Your Amount..."
                style={amtStatus ? styles.input : styles.input1}
                placeholderTextColor={'#8E7B85'}
                autoCorrect={false}
              />
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  <Text
                    ...
                    {item.call_request_received ? 'Ok' : ' Close'}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
