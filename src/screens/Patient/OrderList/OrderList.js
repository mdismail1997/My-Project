...
export const OrderList = (props) => {
    ...
    await Apis.orderlist(data)
      ...
      .then((response) => {
        ...
        for (var i = 0; i <= response.data.response.length; i++) {
          arr.push(response.data.response[i].order_path);
          console.warn(arr);
          SetPrescription(arr);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <Header title="Order List" navProps={props.navigation} />
      ...
      <ScrollView style={{ marginBottom: 20 }}>
        {
          alldata?.length < 1 || alldata == null ?
            (
              <Text style={{ color: '#000', textAlign: 'center', marginTop: 10 }}>No Order Available</Text>
            ) :
            alldata.map((el, i) => (
