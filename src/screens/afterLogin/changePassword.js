...
export default function Otp({navigation, route}) {
  ...
  const sendOTP = async () => {
    ...
    setCounter(60)
    try {
      const email = JSON.parse(await AsyncStorage.getItem('userToken'));
      console.log(
        'email',
        email.data.data.rider
          ? email.data.data.rider.email
    ...
    await axios({
      ...
      .then(response => {
        ...
        if (response.data.success === true) {
          ...
          navigation.navigate('resetPassword', {userEmail: userEmail});
        } else {
          commonToast({
            text: response.data.message,
            position: 'top',
